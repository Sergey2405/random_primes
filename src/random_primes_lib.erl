...
-spec get_env(atom(), atom(), atom(), term()) -> term().
get_env(Server, Application, MapParam, DefaultValue) ->
    case application:get_env(Server, Application, DefaultValue) of
        DefaultValue -> DefaultValue;
        MapValue when is_map(MapValue) -> maps:get(MapParam, MapValue, DefaultValue)
    end.
